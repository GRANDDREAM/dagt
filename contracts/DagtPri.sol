...
//  uint256 public constant RATE1 =  1400;
  ...
  /**
     ...
     */
    function whitelistAddresses( address[] _users) private onlyOwner {
      for( uint i = 0 ; i < _users.length ; i++ ) {
        whiteListedAddress[_users[i]] = true;
      }
    }

    function unwhitelistAddress( address _users) private onlyOwner {
     ...
     * @param _users , the list of user Address. Tested for out of gas until 200 addresses.
     */
    function whitelistAddressesPresale( address[] _users) private onlyOwner {
      for( uint i = 0 ; i < _users.length ; i++ ) {
        whiteListedAddressPresale[_users[i]] = true;
      }
    }

    function unwhitelistAddressPresale( address _users) private onlyOwner {
...
function buyTokensPresale(address beneficiary)  payable {
 ...
 // 不支持小数折扣率返回值是已经乘以100，使用时再除以100
  tokens = tokens+(tokens*rate).div(100);
