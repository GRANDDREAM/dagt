...
contract DagtCrowdSale is CappedCrowdsale, RefundableCrowdsale {
  ...
  uint256 public constant RATE4 = 1118;
  ...
  function DagtCrowdSale(uint256 _startBlock, uint256 _endPresale, uint256 _startSale, uint256 _endBlock, uint256 _goal,uint256 _presaleCap, uint256 _cap, address _wallet) CappedCrowdsale(_cap) FinalizableCrowdsale() RefundableCrowdsale(_goal) Crowdsale(_startBlock, _endBlock, _wallet) public {
    require(_goal <= _cap);
  ...
  function buyTokensPresale(address beneficiary) payable onlyPresaleWhitelisted public {
    require(beneficiary != 0x0);
